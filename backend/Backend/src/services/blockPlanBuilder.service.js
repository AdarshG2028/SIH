// Builds the multi-department shadow-block plan getDemoPlan and the KPI
// endpoint both need — pulled out so KPIs are always computed from the
// exact same plan the demo page shows, not a second copy of this logic.
const {
  loadRiskEnrichedTasks,
  groupMultiDepartmentLocations,
} = require("./multiDepartmentGrouping.service");
const { describeDepartmentSynergy } = require("../shared/departmentSynergy");
const { parseSectionId } = require("../shared/sections");
const { hasSectionData, listFreeWindows } = require("./impact.service");
...
async function buildOptimizedPlan() {
  ...
  for (const group of multiDepartmentGroups) {
    ...
    for (const department of group.departments) {
      const departmentTask = sortedTasks.find((task) => task.department === department);
      if (departmentTask) selectedTasks.push(departmentTask);
    }

    // Fill remaining block capacity with highest-risk jobs.
    for (const task of sortedTasks) {
      if (selectedTasks.length >= 5) break;
      if (!selectedTasks.includes(task)) selectedTasks.push(task);
    }
    ...
    const pushedAsideTasks = sortedTasks.filter((task) => !selectedTasks.includes(task)).slice(0, 3);
    ...
    blocks.push({
      ...
      averageRiskScore:
        selectedTasks.reduce((sum, task) => sum + task.risk.risk_score, 0) / selectedTasks.length,
      ...
      // crew/equipment data to compute a real figure from.
  ...
  const optimizedBlocks = [...blocks]
    .map((block) => ({
      ...block,
      optimizationScore:
        block.averageRiskScore + block.departments.length * 10 + block.tasks.length * 5,
    }))
  ...
  const optimizedPlan = {
    ...
    totalJobs: optimizedBlocks.reduce((sum, block) => sum + block.tasks.length, 0),
    departments: [...new Set(optimizedBlocks.flatMap((block) => block.departments))],
    totalPredictedDelayMinutes: optimizedBlocks.reduce(
      (sum, block) => sum + block.predictedDelayMinutes,
      0,
    ),
    estimatedTotalPrice: optimizedBlocks.reduce((sum, block) => sum + block.estimatedPrice, 0),
    blocks: optimizedBlocks,
  };

  return { blocks, optimizedPlan };
}

module.exports = { buildOptimizedPlan, resolveBlockWindow };

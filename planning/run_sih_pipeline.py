...
# This script was written against the master repo's ML/src/planning layout.
# In this copy the package sits at backend/ML/planning, so the original import
# raised ModuleNotFoundError and the verification never ran here at all.
...
def run_full_sih_verification():
    ...
    # Report against the real backlog and the plan just generated, so the
    # printed KPIs match what the API returns rather than the fallback numbers.
    print_kpi_report(
        optimized_blocks=plan.get("generated_blocks"),
        all_tasks=get_all_department_tasks(corridor="LNL-PUNE"),
        horizon_days=plan.get("horizon_days", 7),
    )

...
# ------------------------------------------------------------------
# Horizon and corridor-window helpers
# ------------------------------------------------------------------

def _resolve_horizon(planning_horizon: str, start_date: str, end_date: str = None) -> list:
    """
    Turns a horizon name into the actual list of nights available for work.

    Without this the horizon parameter was accepted and echoed back unchanged,
    so a "weekly" plan and a "monthly" plan produced identical output. The
    horizon has to become real dates before it can mean anything.
    """
    from datetime import datetime, timedelta
    ...
    span = {"daily": 1, "weekly": 7, "monthly": 30}.get((planning_horizon or "weekly").lower(), 7)
    ...
    return [(start + timedelta(days=i)).isoformat() for i in range(max(1, span))]


def _corridor_quiet_window(corridor_name: str) -> tuple:
    """
    The corridor's own quiet hours from corridor_availability.json.

    Every block used to start at a hardcoded 01:00 regardless of corridor,
    which threw away the one piece of corridor data the file exists to carry.
    """
    info = get_corridor_info(corridor_name) or {}
    windows = (info.get("traffic_profile") or {}).get("quiet_corridor_hours") or []

    if windows:
        try:
            first = windows[0]
            win_start, win_end = [part.strip() for part in first.split("-")]
            capacity = (time_to_minutes(win_end) - time_to_minutes(win_start)) / 60.0
            if capacity > 0:
                return win_start, capacity
...
def _find_clear_window(
    corridor_name: str,
    candidate_nights: list,
    quiet_start: str,
    duration_h: float,
    quiet_capacity_h: float,
    taken: set,
) -> dict:
    """
    Searches nights and start offsets for a window with no train conflicts.

    The planner used to place a block at the corridor's quiet start, notice the
    conflicting goods rakes, subtract points from its own score and schedule it
    anyway. Detecting a conflict and then ignoring it is not optimisation — the
    problem statement asks for uninterrupted train operations, so the block has
    to move. Offsets are tried first (same night, later slot), then later
    nights, and the conflict-free option nearest the requested date wins.
    """
    slack_minutes = max(0, int((quiet_capacity_h - duration_h) * 60))
    offsets = list(range(0, slack_minutes + 1, 30)) or [0]

    best_fallback = None

    for night in candidate_nights:
        for offset in offsets:
            start_min = time_to_minutes(quiet_start) + offset
            start_time = minutes_to_time(start_min)
            end_time = minutes_to_time(start_min + int(duration_h * 60))

            if (corridor_name, night, start_time) in taken:
                continue
            ...
            if not conflicts:
                return {
                    ...
                    "end_time": end_time,
                    "affected_trains": [],
                    "shifted": offset > 0 or night != candidate_nights[0],
                }

            if best_fallback is None or len(conflicts) < len(best_fallback["affected_trains"]):
                best_fallback = {
                    "date": night,
                    "start_time": start_time,
                    "end_time": end_time,
    ...
    # Nothing in the horizon is clear. Return the least-bad slot and let the
    # conflict be reported honestly rather than hidden.
    if best_fallback:
        return best_fallback

    start_min = time_to_minutes(quiet_start)
    return {
        "date": candidate_nights[0],
...
def _bundle_tasks(sorted_tasks: list, max_per_block: int = 3) -> list:
    """
    Splits the full backlog into bundles, highest priority first.

    The planner previously took only the top three tasks per corridor and threw
    the rest away — they were never deferred, never reported, just gone. Every
    task now lands in a bundle and every bundle gets a night.
    """
    return [
        sorted_tasks[i : i + max_per_block]
        for i in range(0, len(sorted_tasks), max_per_block)
    ]
...
def generate_sih_optimized_plan(
    ...
    # 3b. Resolve the horizon into real nights, and note which tasks are due
    #     inside it. A monthly plan now genuinely spans a month of windows.
    nights = _resolve_horizon(planning_horizon, start_date, end_date)
    horizon_start, horizon_end = nights[0], nights[-1]
    ...
    for c_name, c_tasks in corridor_groups.items():
        ...
        quiet_start, quiet_capacity_h = _corridor_quiet_window(c_name)
        bundles = _bundle_tasks(c_tasks_sorted, max_per_block=3)

        for bundle_index, primary_tasks in enumerate(bundles):
            # One bundle per night, in priority order. If the backlog needs more
            # nights than the horizon holds, the overflow is reported as
            # deferred rather than silently dropped.
            if bundle_index >= len(nights):
                deferred_tasks.extend([
                    {
                        "task_id": t.get("task_id"),
                        "department": t.get("department"),
                        "priority_score": priority_map.get(t.get("task_id", ""), {}).get("priority_score", 50),
                        "reason": f"Backlog exceeds the {planning_horizon} horizon of {len(nights)} nights",
                    }
                    for t in primary_tasks
                ])
                continue

            departments_involved = sorted(set(t.get("department", "Engineering") for t in primary_tasks))
            assets_involved = sorted(set(t.get("asset_id", "UNKNOWN") for t in primary_tasks if t.get("asset_id")))

            # Co-located work runs concurrently, so the block is as long as its
            # longest task, capped by what the corridor's quiet window allows.
            block_duration = max(float(t.get("estimated_duration", 2.5)) for t in primary_tasks)
            block_duration = min(block_duration, quiet_capacity_h)

            # Search from this bundle's night onward for a conflict-free slot.
            placement = _find_clear_window(
                corridor_name=c_name,
                candidate_nights=nights[bundle_index:],
                quiet_start=quiet_start,
                duration_h=block_duration,
                quiet_capacity_h=quiet_capacity_h,
                taken=occupied_slots,
            )

            block_date = placement["date"]
            start_time = placement["start_time"]
            end_time = placement["end_time"]
            affected_trains = placement["affected_trains"]
            occupied_slots.add((c_name, block_date, start_time))
            ...
            # Surface the model's own contribution so the plan can be defended.
            ml_scored = [t for t in primary_tasks if t.get("ml_source") == "trained_ensemble"]
            peak_risk = max((float(t.get("ml_probability", 0.0)) for t in ml_scored), default=0.0)

            if not affected_trains:
                conflict_text = (
                    "Shifted clear of all train movements — zero conflicts."
                    if placement["shifted"]
                    else "Zero train conflicts detected."
                )
            else:
                conflict_text = (
                    f"No conflict-free window exists in this horizon; "
                    f"{len(affected_trains)} train(s) flagged for regulation."
                )
            ml_text = (
                f" Highest model-predicted failure probability in this bundle: {peak_risk * 100:.1f}%."
                if ml_scored else ""
            )
            ...
            recommendation_text = (
                ...
                f"{conflict_text} Avoided {downtime_saved:.1f}h of separate corridor closures.{ml_text}"
            )
                ...
                "selected_tasks": [
                    {
                        ...
                        "ml_probability": t.get("ml_probability"),
                        "ml_risk_level": t.get("ml_risk_level"),
                    }
                    for t in primary_tasks
                ],
    ...
    kpi_report = compute_prototype_kpis(generated_blocks, all_tasks=all_tasks, horizon_days=len(nights))

    return {
        "planning_horizon": planning_horizon,
        "horizon_days": len(nights),
        ...
        "tasks_scheduled": sum(len(b["selected_tasks"]) for b in generated_blocks),
        "tasks_deferred": deferred_tasks,
        "ml_scored_tasks": sum(1 for t in all_tasks if t.get("ml_source") == "trained_ensemble"),
